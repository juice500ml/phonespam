...
inventory, resolve it first with :mod:`phonespam.phoible` and
        ...
        *,
        ...
        Args:
            ...
            vocab: optional list of phones to constrain the output to. Phones
                ...
                Phoible inventory, use :mod:`phonespam.phoible`
        ...
        raw_bt = np.asarray(boundaries, dtype=float)
        # Boundaries in the wrong unit are the most common way to use this
        # wrong: segment() hands back frame *indices*, and a frame index used
        # as a timestamp is ~50x too large, so every boundary clips to the
        # utterance end. The label count still matches the input, so nothing
        # downstream notices.
        if raw_bt.size and raw_bt.max() > end_t:
            n_over = int((raw_bt > end_t).sum())
            warnings.warn(
                f"{n_over} of {raw_bt.size} boundaries exceed the utterance duration "
                f"({end_t:.3f}s) and were clipped to it. `boundaries` must be times in "
                f"seconds; Segmenter.segment() returns frame indices, so convert with "
                f"`encoder.frame_to_time(...)` -- or call PhoneModel.transcribe(), "
                f"which handles this.",
                stacklevel=2,
            )
        bt = np.clip(
            np.sort(np.concatenate([[0.0], raw_bt, [end_t]])),
            0.0,
            end_t,
        )
        ...
        # The prototypes are scored against sigmoid activations; raw
        # projections are unbounded and shift a small share of the labels.
        if posteriogram.size and (posteriogram.min() < 0.0 or posteriogram.max() > 1.0):
            warnings.warn(
                f"posteriogram values span [{posteriogram.min():.3f}, "
                f"{posteriogram.max():.3f}], outside the [0, 1] range of a sigmoid "
                f'posteriogram. Project with `act="sigmoid"` -- raw projections '
                f"score against the prototypes differently.",
                stacklevel=2,
            )
